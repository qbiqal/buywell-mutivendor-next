...
module.exports = async function seedCategories() {
  ...
  try {
    // 1. Build rate map: gstRate (percentage) → tax_rate id
    //    tax_rates.total_rate is stored in basis points (500 = 5%)
    const ratesRes = await pool.query('SELECT id, total_rate FROM tax_rates WHERE is_active = true');
    const rateMap = {};
    for (const row of ratesRes.rows) {
      const pct = Math.round(row.total_rate / 100); // 500 → 5, 1200 → 12, etc.
      // Only set if not already set (prevents 28%+cess overwriting 28%)
    ...
    for (const cat of CATEGORIES) {
      ...
      // Insert parent (ON CONFLICT DO NOTHING — keeps admin edits safe)
      await pool.query(`
      ...
      `, [parentId, cat.name, cat.slug, cat.color, cat.description ?? null,
          ...
          cat.showOnHomepage, cat.showOnShop, cat.sortOrder]);

      // Fetch actual ID (may differ if row already existed)
      const { rows: [existing] } = await pool.query(
        'SELECT id FROM product_categories WHERE slug = $1', [cat.slug]
      );
      ...
      for (const child of (cat.children ?? [])) {
        ...
        await pool.query(`
          ...
          ON CONFLICT (slug) DO NOTHING
        `, [randomUUID(), child.name, child.slug, resolvedParentId,
    ...
    const { rows: [cnt] } = await pool.query('SELECT COUNT(*) FROM product_categories');
    console.log(`[category-seed] Done. ${cnt.count} total categories in DB (${parentInserted} parents + ${childInserted} children processed).`);
